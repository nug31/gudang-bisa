  ...
  const checkItemRequests = async () => {
    try {
      ...
      const response = await fetch(
        `/.netlify/functions/check-item-requests?t=${timestamp}`
      );
      ...
      console.error("Error checking item requests:", error);
          ...
          {result.data && (
            <div className="mt-4 space-y-4">
              ...
              {result.data.tableExists && (
                <>
                  <div>
                    <h3 className="font-medium text-sm mb-1">Table Structure</h3>
                      ...
                      <table className="min-w-full divide-y divide-gray-200 text-xs">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left font-medium text-gray-500">Nullable</th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {result.data.tableStructure.map((column: any, index: number) => (
                            <tr key={index}>
                              <td className="px-3 py-2">{column.column_name}</td>
                              <td className="px-3 py-2">{column.data_type}</td>
                              <td className="px-3 py-2">{column.is_nullable === "YES" ? "Yes" : "No"}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  {/* Status Counts */}
                  <div>
                    <h3 className="font-medium text-sm mb-1">Request Status Counts</h3>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      <div>Total Requests:</div>
                      <div>{result.data.totalRequests}</div>
                      
                      {result.data.statusCounts.map((status: any, index: number) => (
                        <React.Fragment key={index}>
                          <div className="capitalize">{status.status || "Unknown"}:</div>
                  ...
                  {result.data.sampleRequests.length > 0 && (
                    <div>
                      <h3 className="font-medium text-sm mb-1">Sample Requests</h3>
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                          <thead className="bg-gray-50">
                            <tr>
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {result.data.sampleRequests.map((request: any, index: number) => (
                              <tr key={index}>
                                <td className="px-3 py-2">{request.id.substring(0, 8)}...</td>
