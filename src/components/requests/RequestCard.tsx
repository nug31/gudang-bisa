  ...
  useEffect(() => {
    try {
      ...
      if (isNaN(requestDate.getTime())) {
        console.warn("Invalid date in request:", request.id, request.createdAt);
        setIsNew(false);
        return;
      }
      ...
      console.error("Error checking if request is new:", e);
      setIsNew(false);
    }
              ...
              {(() => {
                try {
                  ...
                  if (isNaN(date.getTime())) {
                    console.warn(
                      "Invalid date for formatDistanceToNow:",
                      request.id,
                      request.createdAt
                    );
                    return "Unknown time";
                  ...
                  console.error("Error formatting distance to now:", e);
                  return "Unknown time";
                }
              })()}
            </div>
          ...
          {isAdminOrManager && isPending && (
            <Link
              ...
              onClick={(e) => {
                ...
                e.stopPropagation();
                console.log(
                  "Manage Request clicked, navigating to:",
                  `/requests/${request.id}`
                );
              }}
            >
