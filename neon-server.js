  ...
  try {
    switch (action) {
      ...
      case "getById": {
        ...
        try {
          const result = await pool.query(
            "SELECT i.*, c.name as category_name FROM inventory_items i LEFT JOIN categories c ON i.category_id::text = c.id::text WHERE i.id = $1",
            [id]
          );
      ...
      case "delete": {
        ...
        try {
          ...
          const inventoryResult = await pool.query(
            "SELECT COUNT(*) FROM inventory_items WHERE category_id::text = $1::text",
            [id]
          );
