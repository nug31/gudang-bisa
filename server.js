  ...
  try {
    ...
    switch (action) {
      ...
      case "create":
        ...
        try {
          // Insert the new item using PostgreSQL
          ...
          console.log("Category ID type:", typeof newItemCategoryId);

          // Convert categoryId to UUID if it's a number
          let categoryIdParam = newItemCategoryId;
          if (!isNaN(parseInt(newItemCategoryId, 10))) {
            ...
            try {
              ...
              if (categoryResult.rows.length > 0) {
                categoryIdParam = categoryResult.rows[0].id;
              } else {
                console.log("Category not found with ID:", newItemCategoryId);
                // Try to get the first category as fallback
                const fallbackQuery = "SELECT id FROM categories LIMIT 1";
                const fallbackResult = await pool.query(fallbackQuery);
                if (fallbackResult.rows.length > 0) {
                  categoryIdParam = fallbackResult.rows[0].id;
                  console.log("Using fallback category ID:", categoryIdParam);
                }
              }
            } catch (error) {
          ...
          console.log("Item inserted successfully with ID:", newItemId);
          ...
          const newItemResult = await pool.query(itemQuery, [newItemId]);

          if (newItemResult.rows.length === 0) {
            throw new Error("Failed to retrieve newly created item");
          }

          console.log("Retrieved new item:", newItemResult.rows[0].name);
      ...
      case "update":
        ...
        try {
          ...
          updateValues.push(updateItemId);
          ...
          const updatedItemResult = await pool.query(updatedItemQuery, [
            updateItemId,
          ]);
      ...
      case "delete":
        ...
        try {
          ...
          const itemResult = await pool.query(itemQuery, [deleteItemId]);
          ...
          const requestsQuery =
            "SELECT COUNT(*) as count FROM item_requests WHERE inventory_item_id = $1";
          const requestsResult = await pool.query(requestsQuery, [
            deleteItemId,
          ]);
...
// Handle both /db/auth and /api/auth/* endpoints
app.post(["/db/auth", "/api/auth/verify"], async (req, res) => {
  const { action, userId, email, password } = req.body;
  console.log("Auth endpoint accessed:", req.path, "Action:", action);
  ...
  try {
    ...
    switch (action) {
      ...
      case "login":
        // Login user
        console.log("Login attempt via /db/auth:", {
          email,
          password: "********",
        });
        ...
        // Try with PostgreSQL's crypt function as well
        try {
          const cryptResult = await pgPool.query(
            "SELECT * FROM users WHERE email = $1 AND password = crypt($2, password)",
            [email, password]
          );
          const cryptValid = cryptResult.rows.length > 0;
          console.log(`PostgreSQL crypt comparison result: ${cryptValid}`);
          isPasswordValid = isPasswordValid || cryptValid;
        } catch (cryptError) {
          console.error("Error comparing passwords with crypt:", cryptError);
        }
