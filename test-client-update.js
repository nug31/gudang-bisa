...
async function testClientUpdate() {
  try {
    console.log("Testing client-side update...");
    ...
    if (!getAllResponse.ok) {
      throw new Error(
        `Failed to get requests: ${getAllResponse.status} ${getAllResponse.statusText}`
      );
    }
    ...
    const newStatus =
      requestToUpdate.status === "pending" ? "approved" : "pending";
    console.log(
      `Updating request status from ${requestToUpdate.status} to ${newStatus}`
    );

    // Simulate the client-side update process
    // This is similar to what happens in the RequestContext.tsx updateRequest function
    ...
    // Send the update request using the same method as the client
    ...
    console.error("Error testing client update:", error);
  }
}
