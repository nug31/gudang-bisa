...
import { ArrowLeft, RefreshCw, Database, CheckCircle, XCircle, AlertTriangle } from "lucide-react";
...
const ItemRequestsTest: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/.netlify/functions/item-requests-direct?t=${timestamp}`);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6 mt-6">
          ...
          {loading && !data ? (
            ...
            <div>
              <div className={`flex items-center mb-4 ${data.success ? "text-green-600" : "text-red-600"}`}>
                {data.success ? (
              ...
              {data.tableExists ? (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <h3 className="font-medium text-gray-700 mb-3">Request Counts</h3>
                      <div className="space-y-2">
                        ...
                        {data.statusCounts && data.statusCounts.map((item: any, index: number) => (
                    ...
                    {/* Recent Requests */}
                    <div className="bg-gray-50 rounded-lg p-4">
                      <h3 className="font-medium text-gray-700 mb-3">Recent Requests</h3>
                      {data.recentRequests && data.recentRequests.length > 0 ? (
                        <div className="space-y-3">
                          {data.recentRequests.map((request: any, index: number) => (
                            <div key={index} className="border-b pb-2 last:border-0">
                              <div className="flex justify-between">
                                <span className="font-medium">{request.title || "Untitled"}</span>
                        ...
                        <div className="text-gray-500 italic">No recent requests found</div>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 text-yellow-700">
                  <div className="flex items-center">
                    ...
                    <span>The item_requests table does not exist in the database.</span>
                  </div>
                </div>
              )}
